...
| container label | `com.mrbaron3.servo.*`が唯一のownership namespace。writerもreaderもこれだけを扱う（[#123](https://github.com/mrbaron3/servo/issues/123) Phase 3B済） | 旧`com.mrbaron3.workflow.*`はproduction codeから完全に消えた。旧namespaceだけのresourceは非所有として触らない。current namespaceが中途半端なものはunowned扱いせずfail-closed。P1/P2/P3Aへのrollbackは保全済みprivate backupと旧binaryを要する意図的判断。運用は[runbook](runbooks/container-label-migration.md) |
